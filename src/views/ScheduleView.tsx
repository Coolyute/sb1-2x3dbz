...
export function ScheduleView() {
        ...
        {event.categories.map((category, index) => {
          ...
          return (
            <div
              ...
              onClick={() => setSelectedEvent({ name: event.name, category })}
            >
  ...
  const heats = events.filter(e => e.type === 'heat');
    ...
    <div className="p-6">
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div>
          ...
          {heats.map((event, index) => renderEvent(event, index + 2))}
        </div>
        ...
        <div>
          ...
          {midDistanceEvents.map((event, index) => renderEvent(event, index + heats.length + 2))}
        </div>
        ...
        <div>
          ...
          {regularFinals.map((event, index) => 
            renderEvent(event, index + heats.length + midDistanceEvents.length + 3)
          )}
        </div>
